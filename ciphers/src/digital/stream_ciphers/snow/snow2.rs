...
fn sbox(n: u32) -> u32 {
    S3[n as usize & 0xff]
        | S2[(n as usize) >> 8 & 0xff]
        | S1[(n as usize) >> 16 & 0xff]
        | S0[(n as usize) >> 24 & 0xff]
}
...
impl Snow2 {
    pub fn with_key_and_iv_128(key: [u8; 16], iv: [u32; 4]) -> Self {
        ...
        s.words[12] = !s.words[0];
        s.words[13] = !s.words[1];
...
crate::impl_cipher_for_stream_cipher!(Snow2);
