...
crate::block_cipher_builders! {FealNx, u64}
...
impl BlockCipher<8> for FealNx {
    fn encrypt_block(&self, bytes: &mut [u8]) {
        ...
        // Preprocessing stage
        v[0] ^= (self.subkeys[ROUNDS] as u32) << 16 | self.subkeys[ROUNDS + 1] as u32;
        v[1] ^= (self.subkeys[ROUNDS + 2] as u32) << 16 | self.subkeys[ROUNDS + 3] as u32;
        v[1] ^= v[0];
        debug_state!("e pre:", v);

        // Feistel network
        for subkey in self.subkeys.into_iter().take(ROUNDS) {
            let t = v[0];
            // L_i+1 = R_i
            v[0] = v[1];
        ...
        v.swap(0, 1);
        v[1] ^= v[0];
        v[0] ^= (self.subkeys[ROUNDS + 4] as u32) << 16 | self.subkeys[ROUNDS + 5] as u32;
        v[1] ^= (self.subkeys[ROUNDS + 6] as u32) << 16 | self.subkeys[ROUNDS + 7] as u32;
    ...
    fn decrypt_block(&self, bytes: &mut [u8]) {
        ...
        // Feistel network
        for subkey in self.subkeys.into_iter().take(ROUNDS).rev() {
            let t = v[1];
            // L_i+1 = R_i
            v[1] = v[0];
...
// https://info.isl.ntt.co.jp/crypt/eng/archive/feal/specifications.html
crate::test_block_cipher!(
    test_1, FealNx::default().with_key([
        0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab,
        0xcd, 0xef,
    ]),
    [0, 0, 0, 0, 0, 0, 0, 0],
    ...
    test_2, FealNx::default().with_key([
