use utils::byte_formatting::{fill_u32s_be, make_u32s_be, u32s_to_bytes_be, ByteFormat};

use super::block_cipher::{BCMode, BCPadding, BlockCipher};

const SM4_SBOX: [u8; 256] = [
    ,
];

const FAMILY_KEY: [u32; 4] = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc];

const C: [u32; 32] = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279,
];

fn tau(a: u32) -> u32 {
    u32::from_be_bytes(a.to_be_bytes().map(|b| SM4_SBOX[b as usize]))
}

fn lin(a: u32) -> u32 {
    a ^ a.rotate_left(2) ^ a.rotate_left(10) ^ a.rotate_left(18) ^ a.rotate_left(24)
}

fn lin_prime(a: u32) -> u32 {
    a ^ a.rotate_left(13) ^ a.rotate_left(23)
}

fn t(a: u32) -> u32 {
    lin(tau(a))
}

fn t_prime(a: u32) -> u32 {
    lin_prime(tau(a))
}

fn f(state: &[u32; 4], key: u32) -> u32 {
    state[0] ^ t(state[1] ^ state[2] ^ state[3] ^ key)
}

fn f_prime(state: &[u32; 4], key: u32) -> u32 {
    state[0] ^ t_prime(state[1] ^ state[2] ^ state[3] ^ key)
}

pub struct Sm4 {
    pub input_format: ByteFormat,
    pub output_format: ByteFormat,
    pub mode: BCMode,
    pub padding: BCPadding,
    pub iv: u128,
    pub subkeys: [u32; 32],
}

impl Default for Sm4 {
    fn default() -> Self {
        Self {
            input_format: ByteFormat::Hex,
            output_format: ByteFormat::Hex,
            mode: BCMode::default(),
            padding: BCPadding::default(),
            iv: 0,
            subkeys: Default::default(),
        }
    }
}

crate::block_cipher_builders! {Sm4, u128}

impl Sm4 {
    pub fn ksa_32(&mut self, key: [u32; 4]) {
        let mut k = key;
        for i in 0..4 {
            k[i] ^= FAMILY_KEY[i];
        }

        for i in 0..32 {
            let t = f_prime(&k, C[i]);
            self.subkeys[i] = t;
            k.rotate_left(1);
            k[3] = t;
        }
    }

    pub fn ksa(&mut self, bytes: [u8; 16]) {
        let mut k = [0; 4];
        fill_u32s_be(&mut k, &bytes);
        for i in 0..4 {
            k[i] ^= FAMILY_KEY[i];
        }

        for i in 0..32 {
            let t = f_prime(&k, C[i]);
            self.subkeys[i] = t;
            k.rotate_left(1);
            k[3] = t;
        }
    }

    pub fn with_key(mut self, bytes: [u8; 16]) -> Self {
        self.ksa(bytes);
        self
    }
}

impl BlockCipher<16> for Sm4 {
    fn encrypt_block(&self, bytes: &mut [u8]) {
        let mut block = make_u32s_be::<4>(bytes);

        for key in self.subkeys {
            let x = f(&block, key);
            block.rotate_left(1);
            block[3] = x;
        }
        block.rotate_right(1);
        block.swap(1, 3);

        u32s_to_bytes_be(bytes, &block);
    }

    fn decrypt_block(&self, bytes: &mut [u8]) {
        let mut block = make_u32s_be::<4>(bytes);

        for key in self.subkeys.into_iter().rev() {
            let x = f(&block, key);
            block.rotate_left(1);
            block[3] = x;
        }
        block.rotate_right(1);
        block.swap(1, 3);

        u32s_to_bytes_be(bytes, &block);
    }
}

crate::impl_cipher_for_block_cipher!(Sm4, 16);

crate::test_block_cipher!(
    test_1, Sm4::default().with_key([0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10]),
    [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10],
    [0x68, 0x1e, 0xdf, 0x34, 0xd2, 0x06, 0x96, 0x5e, 0x86, 0xb3, 0xe9, 0x4f, 0x53, 0x6e, 0x42, 0x46];

    test_2, Sm4::default().with_key([0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef]),
    [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f],
    [0xf7, 0x66, 0x67, 0x8f, 0x13, 0xf0, 0x1a, 0xde, 0xac, 0x1b, 0x3e, 0xa9, 0x55, 0xad, 0xb5, 0x94];
);
