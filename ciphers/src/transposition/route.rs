...
impl Default for Route {
    fn default() -> Route {
        Route {
            alphabet: PresetAlphabet::BasicLatin,
            dimensions: (4, 5),
...
impl Route {
    ...
    pub fn control_key(&mut self) -> &mut String {
        self.key = rank_str(&self.keyword, self.alphabet.slice());
        &mut self.keyword
    }

    pub fn set_key(&mut self, keyword: &str) {
...
impl Cipher for Route {
    ...
    fn randomize(&mut self, rng: &mut StdRng) {
        self.keyword = random_sample_replace(self.alphabet.slice(), self.dimensions.1, rng);
        self.key = rank_str(&self.keyword, self.alphabet.slice());
    }
