...
impl DecoderRing {
    ...
    fn valid_code_group(&self, s: &str) -> Result<usize, GeneralError> {
        match s.parse::<usize>() {
            Ok(n) => {
                if n < self.length() {
                    Ok(n)
                } else {
                    Err(GeneralError::invalid_input_group(s))
                }
            }
            Err(_) => return Err(GeneralError::invalid_input_group(s)),
        }
    }
}
...
mod decoder_ring_tests {
    ...
    const CTEXT: &'static str = "21 11 18 23 26 8 2 13 20 1 17 7 22 12 17 14 19 26 9 16 5 17 10 18 1 21 11 18 6 4 24 0 15 17 25";
