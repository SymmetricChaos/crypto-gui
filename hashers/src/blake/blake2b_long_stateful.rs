...
impl Blake2bLongStatful {
    ...
    pub fn init_hash(hash_len: u64) -> Self {
        let mut h = if hash_len <= 64 {
            Blake2bStateful::init(&[], hash_len)
        } else {
            Blake2bStateful::init(&[], 64)
        };
...
impl StatefulHasher for Blake2bLongStatful {
