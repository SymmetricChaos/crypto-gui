...
impl Keccak {
    ...
    pub fn permutation(state: &mut KeccackState, round: usize) {
        ...
        let mut d = [0; 4];
        for x in 0..4 {
            // TODO: check this wrapps the index correctly
            d[x] = c[(x + 3) % 4] ^ c[x + 2].rotate_right(1); // TODO: dheck rotation direction
        }
        for x in 0..4 {
            for y in 0..4 {
                state.array[x][y] ^= d[x]
        ...
        for x in 0..4 {
            for y in 0..4 {
                b[y][(2 * x + 3 * y) & 5] =
                    state.array[x][y].rotate_right(Self::ROTATION_CONSTANTS[x][y])
            }
        }
