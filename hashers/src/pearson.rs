...
pub const PEARSON_ARRAY: [u8; 256] = [
...
impl StatefulHasher for Pearson {
    fn update(&mut self, bytes: &[u8]) {
        for byte in bytes {
            self.state = self.array[(self.state ^ byte) as usize]
        }
    }
...
crate::stateful_hash_tests!(
    test1, Pearson::init_default(), b"the", "9d";
);
