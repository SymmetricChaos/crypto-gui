...
pub(crate) fn padded_bytes_64(bytes: &[u8]) -> u64 {
    assert!(bytes.len() <= 8);
    if bytes.len() == 8 {
...
// pub(crate) fn unpadded_bytes_64(bytes: &[u8]) -> u64 {
//     assert!(bytes.len() <= 8);
//     if bytes.len() == 8 {
//         u64::from_be_bytes(bytes.try_into().unwrap())
//     } else {
...
// pub(crate) fn unpadded_bytes_128(bytes: &[u8]) -> [u64; 2] {
//     assert!(bytes.len() <= 16);
//     if bytes.len() == 16 {
//         [
...
pub(crate) fn padded_bytes_256(bytes: &[u8]) -> [u64; 4] {
    assert!(bytes.len() <= 32);
    if bytes.len() == 32 {
        [
            u64::from_be_bytes(bytes[0..8].try_into().unwrap()),
...
pub(crate) fn padded_bytes_320(bytes: &[u8]) -> [u64; 5] {
    assert!(bytes.len() <= 40);
    if bytes.len() == 40 {
        [
            u64::from_be_bytes(bytes[0..8].try_into().unwrap()),
...
#[derive(Debug, Clone)]
pub struct AsconState(pub(crate) [u64; 5]);
...
impl AsconState {
    ...
    pub fn absorb_64_partial(&mut self, buffer: &mut Vec<u8>, a: usize) {
        let mut mlen = buffer.len();
        let mut ptr = 0;
        // Absorb full blocks
        while mlen >= RATE {
            self[0] ^= padded_bytes_64(&buffer[ptr..ptr + RATE]);
            ptr += RATE;
            mlen -= RATE;
            self.rounds(a);
        ...
        *buffer = buffer[ptr..].to_vec();
    }

    pub fn absorb_64_final(&mut self, buffer: &mut Vec<u8>) {
        // Absorb the last padded block
        self[0] ^= padded_bytes_64(&buffer[0..]);
        self.rounds_12();
        if DEBUG {
    ...
    pub fn absorb_256_partial(&mut self, buffer: &mut Vec<u8>, a: usize) {
        let rate = 32;
        ...
        // Absorb full blocks
        while mlen >= rate {
            let [x0, x1, x2, x3] = padded_bytes_256(&buffer[ptr..ptr + rate]);
            self[0] ^= x0;
            self[1] ^= x1;
            self[2] ^= x2;
            self[3] ^= x3;
    ...
    pub fn absorb_256_final(&mut self, buffer: &mut Vec<u8>) {
        // Absorb the last padded block
        let [x0, x1, x2, x3] = padded_bytes_256(&buffer[0..]);
        self[0] ^= x0;
        self[1] ^= x1;
        self[2] ^= x2;
        self[3] ^= x3;
    ...
    pub fn absorb_320_partial(&mut self, buffer: &mut Vec<u8>, a: usize) {
        let rate = 40;
        ...
        // Absorb full blocks
        while mlen >= rate {
            let [x0, x1, x2, x3, x4] = padded_bytes_320(&buffer[ptr..ptr + rate]);
            self[0] ^= x0;
            self[1] ^= x1;
            self[2] ^= x2;
            self[3] ^= x3;
    ...
    pub fn absorb_320_final(&mut self, buffer: &mut Vec<u8>, a: usize) {
        // Absorb the last padded block
        let [x0, x1, x2, x3, x4] = padded_bytes_320(&buffer[0..]);
        self[0] ^= x0;
        self[1] ^= x1;
        self[2] ^= x2;
        self[3] ^= x3;
    ...
    pub fn squeeze_64(&mut self, hash_len: usize, a: usize) -> Vec<u8> {
    ...
    pub fn squeeze_128(&mut self, hash_len: usize, a: usize) -> Vec<u8> {
        let mut output = Vec::with_capacity(hash_len);
