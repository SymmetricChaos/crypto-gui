...
impl TurningGrille {
    ...
    pub fn build_grid(&mut self) -> Result<(), CodeError> {
        ...
        for key in &self.keys {
            for n in key {
                if n >= &self.subgrille_size() {
                    return Err(CodeError::Key(format!("invalid key value found: {}", n)));
                }
                if !set.insert(n) {
                    return Err(CodeError::Key(format!("duplicate key value found: {}", n)));
                }
            }
        }
