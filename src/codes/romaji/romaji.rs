...
mod romaji_test {
    ...
    fn hepbern_text() {
        ...
        assert_eq!(
            to_romaji(HIRAGANA, HEPBERN_SHIKI.clone()).unwrap(),
            latin_hs
        );
    }
    ...
    fn nihon_text() {
        ...
        assert_eq!(to_romaji(HIRAGANA, NIHON_SHIKI.clone()).unwrap(), latin_ns);
    }

    #[test]
    fn kunrei_text() {
        let latin_ks = "TI SI TU ZI ZI ZU ZYA ZYA KONNI KON'O";
        assert_eq!(to_romaji(HIRAGANA, KUNREI_SHIKI.clone()).unwrap(), latin_ks);
    }
}
