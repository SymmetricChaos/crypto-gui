...
impl VerhoeffAlgorithm {
    fn mul(a: char, b: char) -> Result<&'static char, Error> {
        VERHOEFF_MUL_TABLE.get(&(a, b)).ok_or(Error::Input(format!(
            "invalid mul input pair ({},{})",
            a, b
        )))
    }

    fn perm(n: usize, a: char) -> Result<&'static char, Error> {
        VERHOEFF_PERM_TABLE
            .get(&(n, a))
            .ok_or(Error::invalid_input_char(a))
    }

    fn inv(a: &char) -> Result<&'static char, Error> {
        VERHOEFF_INV_TABLE
            .get(a)
            .ok_or(Error::invalid_input_char(*a))
    }
...
impl Code for VerhoeffAlgorithm {
    ...
    fn decode(&self, text: &str) -> Result<String, Error> {
        ...
        if check != '0' {
            return Err(Error::input("invalid check digit"));
        } else {
