use eframe::egui::{SidePanel, Context, CentralPanel};
...
impl TextPrepPage {
    ...
    fn remove_characters(&mut self) {
        self.text = self.text.chars().filter(|c| !self.remove.contains(*c)).collect()
    }
    ...
    pub fn view(&mut self, ctx: &Context) {
        SidePanel::left("text_prep_page_buttons")
            ...
            .show(ctx, |ui| {
                ...
                ui.collapsing("Advanced", |ui| {
                    let normalize = ui.button("Normalize").on_hover_text_at_pointer("Normalize Unicode Representation");
        ...
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.label(format!("Bytes:      {}",self.num_bytes)).on_hover_text_at_pointer("Number of bytes used to represent the text as a UTF-8 string");
			ui.label(format!("Characters: {}",self.num_chars)).on_hover_text_at_pointer("Number of Unicode codepoints in the string");
			ui.label(format!("Graphemes:  {}",self.num_graphemes)).on_hover_text_at_pointer("Number of Unicode graphemes in the string");
        });
    }
}