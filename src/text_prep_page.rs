    ...
    pub fn view(&mut self, ctx: &Context) {
        SidePanel::left("text_prep_page_buttons")
            ...
            .show(ctx, |ui| {
			    ...
			    if ui.button("Remove All Below").clicked() {
			        self.replace();
			    }
			    ui.label("characters:");
			    ...
			    if ui.button("Retain Only Below").clicked() {
			        self.replace();
			    }
			    ui.label("characters:");
        ...
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.label(format!("Bytes:      {}",self.num_bytes)).on_hover_text("Number of bytes used to represent the text as a UTF-8 string");
			ui.label(format!("Characters: {}",self.num_chars)).on_hover_text("Number of Unicode codepoints in the string");
			ui.label(format!("Graphemes:  {}",self.num_graphemes)).on_hover_text("Number of Unicode graphemes that the string contains");
        });
    }
}